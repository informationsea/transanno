...
use crate::poslift::{LiftPosition, PositionLiftOver};
use crate::variantlift::error::VariantLiftOverError;
use crate::variantlift::{LiftVariant, LiftedVariant, VariantLiftOver};
use crate::vcfparse::{
...
#[derive(Debug)]
pub struct VCFLiftOver<V: LiftVariant + LiftPosition> {
    variant_lift: V,
...
fn lift_header_show_warnings(
    ...
    if !rewrite_target.allele_count.is_empty()
        && !param.do_not_swap_ref_alt
        && !param.do_not_rewrite_allele_count
    {
        warn!("Rewriting allele count is not implemented.");
        for one in rewrite_target.allele_count.iter() {
            rewrite_target.info_alt.insert(one.clone());
        }
    }

    // if rewrite_target.format_gt && !param.do_not_swap_ref_alt && !param.do_not_rewrite_gt {
    //     warn!("Rewriting GT is not implemented.");
    // }
}

impl<V: LiftVariant + LiftPosition> VCFLiftOver<V> {
    pub fn new(variant_lift: V, param: VCFLiftOverParameters) -> Self {
        VCFLiftOver {
            variant_lift,
            param,
        }
    ...
    pub fn lift_header(
        ...
        let mut chromosomes = self.variant_lift.query_chromosomes().to_vec();
...
impl VCFLiftOver<VariantLiftOver<fasta::IndexedReader<File>, PositionLiftOver>> {
    pub fn load<P1: AsRef<Path>, P2: AsRef<Path>, P3: AsRef<Path>>(
        chain_file_path: P1,
        reference_sequence: P2,
        query_sequence: P3,
        param: VCFLiftOverParameters,
    ) -> Result<Self, LiftOverError> {
        info!("start loading chain file");
        let mut reference_sequence_reader = fasta::IndexedReader::from_file(&reference_sequence)?;
        let mut query_sequence_reader = fasta::IndexedReader::from_file(&query_sequence)?;
        let chain_file: Box<dyn Read> =
            if chain_file_path.as_ref().to_str().unwrap().ends_with(".gz") {
                debug!("gz file mode: {:?}", chain_file_path.as_ref());
                Box::new(flate2::read::MultiGzDecoder::new(File::open(
                    chain_file_path,
                )?))
            } else {
                debug!("plain file mode: {:?}", chain_file_path.as_ref());
                Box::new(File::open(chain_file_path)?)
            };
        let chain = ChainFile::load(chain_file)?;
        let left_aligned_chain = if param.do_not_left_align_chain_file {
            chain
        } else {
            chain.left_align(&mut reference_sequence_reader, &mut query_sequence_reader)?
        };
        info!("finish loading chain file");
        Ok(VCFLiftOver::new(
            VariantLiftOver::new(
                left_aligned_chain,
                reference_sequence_reader,
                query_sequence_reader,
            ),
            param,
        ))
    }
