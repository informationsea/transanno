...
mod test {
    ...
    lazy_static! {
        ...
        static ref CHEK2_EXON12: Gff3Record = Gff3Record {
            ...
            attributes: indexmap! {
                ...
                "gene_id".to_string() => "ENSG00000183765.21".to_string(),
                ...
                "transcript_name".to_string() => "CHEK2-204".to_string(),
    ...
    fn test_gff3_parse() {
        assert_eq!(
            "chr22\tHAVANA\tgene\t16503304\t16509475\t.\t+\t.\tID=ENSG00000235969.1;gene_id=ENSG00000235969.1;gene_type=unprocessed_pseudogene;gene_name=CHEK2P4;level=2;havana_gene=OTTHUMG00000140400.1\n".parse::<Gff3Record>().unwrap(),
            *CHEK2P4_GENE
        );

        assert_eq!(
            "chr22\tHAVANA\tCDS\t28695710\t28695873\t.\t-\t0\tID=CDS:ENST00000382580.6;Parent=ENST00000382580.6;gene_id=ENSG00000183765.21;transcript_id=ENST00000382580.6;gene_type=protein_coding;gene_name=CHEK2;transcript_type=protein_coding;transcript_name=CHEK2-204;exon_number=12;exon_id=ENSE00003559542.1;level=2;protein_id=ENSP00000372023.2;transcript_support_level=1;tag=basic,appris_alternative_2,CCDS;ccdsid=CCDS33629.1;havana_gene=OTTHUMG00000151023.24;havana_transcript=OTTHUMT00000321016.2\n".parse::<Gff3Record>().unwrap(),
            *CHEK2_EXON12
        );
    }
    ...
    fn test_gff3_to_string() {
        assert_eq!(
            "chr22\tHAVANA\tgene\t16503304\t16509475\t.\t+\t.\tID=ENSG00000235969.1;gene_id=ENSG00000235969.1;gene_type=unprocessed_pseudogene;gene_name=CHEK2P4;level=2;havana_gene=OTTHUMG00000140400.1\n",
            CHEK2P4_GENE.to_string()
        );

        assert_eq!(
            "chr22\tHAVANA\tCDS\t28695710\t28695873\t.\t-\t0\tID=CDS:ENST00000382580.6;Parent=ENST00000382580.6;gene_id=ENSG00000183765.21;transcript_id=ENST00000382580.6;gene_type=protein_coding;gene_name=CHEK2;transcript_type=protein_coding;transcript_name=CHEK2-204;exon_number=12;exon_id=ENSE00003559542.1;level=2;protein_id=ENSP00000372023.2;transcript_support_level=1;tag=basic,appris_alternative_2,CCDS;ccdsid=CCDS33629.1;havana_gene=OTTHUMG00000151023.24;havana_transcript=OTTHUMT00000321016.2\n",
            CHEK2_EXON12.to_string()
        );
    }
    ...
    fn test_gff3_reader() {
        ...
        let values: Vec<_> = gff3_reader.collect();
        assert_eq!(values[0].as_ref().unwrap(), &*CHEK2P4_GENE);
        assert_eq!(values[199].as_ref().unwrap(), &*CHEK2_EXON12);
        assert_eq!(values.len(), 340);
    }
    ...
    fn test_gff3_grouped_reader() {
        ...
        let values: Vec<_> = grouped_gff3_reader.collect();

        assert_eq!(values[0].as_ref().unwrap().original_record, *CHEK2P4_GENE);
